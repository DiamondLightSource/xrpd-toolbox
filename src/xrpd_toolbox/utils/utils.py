...
def gaussian(x, amp, cen, fwhm, background) -> np.ndarray:
    ...
    return (amp/(np.sqrt(2*np.pi)*fwhm)) * np.exp(-(x-cen)**2 /(2*fwhm**2))+background
...
def create_bins(tth_values: np.ndarray, rebin_step) -> tuple[np.ndarray, np.ndarray]:
...
def bin_and_propagate_errors(x: np.ndarray, y: np.ndarray ,e: np.ndarray, rebin_step: float = 0.004, error_calc: str ='best', sum_counts: bool = False) ->  tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if x[-1] == bin_edges[-1]: #if the last value is exactly equal to the final bin edge it will be lost. 
        x[-1] = x[-1]-(rebin_step/10000) #I think it would be better to move it inside bin edge, and include, rather than remove all together or create a bin with a single value

    sums, bin_edges = np.histogram(x, bins=bin_edges, weights=y)
    counts = np.histogram(x, bins=bin_edges)[0]
    
    if sum_counts:
        binned_counts = sums  #this will throw a warning if there are missing counts in a bin as a result of missing module
    else:
        binned_counts = sums / counts  #this will throw a warning if there are missing counts in a bin as a result of missing module
