...
#  Column description:  a1 a2 a3 a4 a5 b1 b2 b3 b4 b5
COMPTON = {
    "H": [
        0.2623,
        0.5094,
...
def calculate_form_factor_for_element(element: str, s_arr: np.ndarray) -> np.ndarray:
    """Vectorised Waasmaier–Kirfel form factor over an s array."""
    a_list, b_list, c = X_RAY_FORM_FACTORS[element]
    ff = np.full(len(s_arr), 0, dtype=np.float64)
    for a, b in zip(a_list, b_list, strict=True):
        ff += a * np.exp(-b * s_arr**2)
    ff = ff + c
    return ff
...
def calculate_form_factor(elements: Collection[str], s: np.ndarray) -> np.ndarray:
    ...
    return np.array([calculate_form_factor_for_element(e, s) for e in elements])


def calculate_compton_for_element(element: str, s_arr: np.ndarray) -> np.ndarray:

    element_coeffs = COMPTON[element]
    z = ELEMENT_ATOMIC_NUMBER[element]

    a_coeffs, b_coeffs = np.array_split(element_coeffs, 2)

    compton = np.full(len(s_arr), 0, dtype=np.float64)
    for a, b in zip(a_coeffs, b_coeffs, strict=True):
        compton += a * np.exp(-b * s_arr**2)

    compton = z - compton
    return compton


def calculate_compton(elements: Collection[str], s: np.ndarray) -> np.ndarray:

    return np.array([calculate_compton_for_element(e, s) for e in elements])
...
if __name__ == "__main__":
    ...
    for _ in range(iters):
        calculate_form_factor(elements, s)

    print(time.time() - start1)

    print(calculate_compton_for_element("Si", s))
