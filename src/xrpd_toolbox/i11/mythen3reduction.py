...
import json
# from collections import OrderedDict add this later for module angular cal
...
try:
	...
	from daqmessenger import DaqMessenger #type: ignore
...
def NormalizeTo(data: np.ndarray, minval: int = 0):

	"""
	minval isn't the minimum value that will come out of the processed array, what the scale of normlisation is on
	"""
...
class AnalysisLogger():
	def __init__(self, log_filepath, logging=False):
		...
		elif os.path.exists(self.log_filepath) and (os.path.getsize(self.log_filepath) > 1e7):
...
class I11_reduction():
	...
	__slots__ = ('filepath', 'reduced_nxs_filepath_out', 'xye_filepath_out', 'xye_filepath_out_Q', 'file_dir', 'out_directory','file_name', 'file_extension', 'filename_suffix', \
		'config_filepath', 'STRIPS_PER_MODULE', 'MODULES_PER_DETECTOR', 'config', 'mythen3_config_dir', \
		'flatfield_filepath', 'apply_flatfield', 'active_modules', 'bad_modules', 'bad_frames' ,'bad_channel_masking', 'default_counter', 'n_bad_edge_channels', \
		'rebin_step', 'error_calc', 'beam_energy', 'beamline_offset', 'save_nxs_out', 'verbose_nxs', 'debug_mode', 'save_in_Q_space', 'data_reduction_mode', 'wavelength', \
		'raw_flatfield_counts', 'flatfield_modules', 'bad_channels', 'module_angular_cal', 'module_raw_tth', 'deltas', \
		'duration', 'n_frames', 'wholedetector_raw_frames', 'raw_frame_counts', 'frames_range', \
		'all_module_data', 'angular_corrected_data', 'module_raw_data', 'xyedata', 'frame_data', 'angular_corrected_data_unmasked','out_raw_data','n_modules_in_data', 'Ie', 'Ic4',\
	...
	def read_singular_angcal_files(angcal_filepath: str) -> tuple[dict, float |  None]:
		...
		Reads a single of ang.off files and returns a dict with the each modules anngular calibrations contains within a dict
		...
		with open(angcal_filepath) as f:
			...
			for line in f:
				...
				if "beamline_offset" in line:
					...
					module_in_file, module_cal["offset"], module_cal["conv"], module_cal["centre"] = \
					int(elements[1]), float(elements[3]), float(elements[5]), float(elements[7])
	...
	@staticmethod
	def read_angular_calibration_and_create_cal_dict(config: dict, active_modules: list[int]) -> dict:

		"""
	
		Reads a load of ang_d .off files and returns a dict with the each modules anngular calibrations contains within a dict
		...
		for active_mod in active_modules:
			...
			single_cal_file = config["".join(["module_",str(active_mod)])]["angular_calibration_path"]
			...
			with open(single_cal_file) as f:
				...
				for line in f:
					...
					if "beamline_offset" in line:
						...
						# if module_in_file != active_mod:
						# 	self.logger.log("Angular calibration specified in config.toml, doesn't agree with module specified in .off file. Is this what you want?")
	...
	# def read_angular_cal_json(self, filepath):
	# 	with open(filepath) as f:
	# 		ang_cal_load = json.load(f)

	# 	print(ang_cal_load)

	# 	beamline_offset = ang_cal_load["beamline_offset"]
	...
	@staticmethod
	def channel_to_angle_in_real_units(module_pixel_number, centre, offset, beamline_offset, radius, p=0.05):
		"""
		...
		raw_tth = I11_reduction.channel_to_angle(module_pixel_number, centre, (p/radius), offset, beamline_offset)
	...
	def calculate_modules_tth(self):
		...
		for n_mod in self.active_modules:
			...
			raw_tth = I11_reduction.channel_to_angle(module_pixel_number,
			...
			# module_conversions = module_pixel_number-self.module_angular_cal[n_mod]["centre"]
			# # module_conversions = module_pixel_number-self.module_angular_cal[n_mod]["centre"]

			# module_conversions = module_conversions*self.module_angular_cal[n_mod]["conv"]
			# module_conversions = np.arctan(module_conversions)
			# raw_tth = self.module_angular_cal[n_mod]["offset"]+np.rad2deg(module_conversions)+self.beamline_offset

			module_raw_tth[n_mod] = raw_tth #this is raw because it doesn't take into account the angular delta of the detector

		whole_data_raw_tth = np.array([module_raw_tth[f] for f in self.active_modules]).flatten()
	...
	def generate_badchannel_dict(self) -> dict:
		...
		loads the bad channels specified with the bad_channels_path which are specified in the .toml config file.

		Creates a dictionary of bad channels so that they can be accessed self.bad_channels[n] where n is 0-27
		...
		for active_mod in self.active_modules:
			badchan_file = self.config["".join(["module_",str(active_mod)])]["bad_channels_path"]
			bad_channels[active_mod] = I11_reduction.load_int_array_from_file(badchan_file)
			self.logger.log(f"Module: {active_mod} badchan_file | Bad Chans: {len(bad_channels[active_mod])}")
	...
	def apply_flatfield_correction(self, no_ff_corr: np.ndarray, flatfield_counts: np.ndarray) -> np.ndarray:
		"""
	...
	def read_nxs_metadata(self, filepath: str):
		...
		with h5pyFile(filepath, "r") as file:
			...
			# self.logger.log(f"\nData file keys and shape: {entry["mythen_nx"].keys()} {entry["mythen_nx"]["data"]}")
			# self.logger.log("Data shape: [(frames), (data), (counter)]")

			self.n_frames = (len(entry["mythen_nx"]["data"]))
			first_frame_len = len(entry["mythen_nx"]["data"][0, :, self.default_counter][()])
	...
	def read_nxs_data(self, filepath: str, frame: int | None = None, sum_frames: bool = False, bad_frames = []) -> dict:
		...
		with h5pyFile(filepath, "r") as file:
			...
			if (frame != None) and (sum_frames == False):

				#for when we want to only read a specific frame - ie time resolved

				self.wholedetector_raw_frames = entry["mythen_nx"]["data"][frame, :, self.default_counter][()] # [n, 17920, 3] where n is the number of frames, 3 = 3 counters
				self.wholedetector_raw_frames = self.wholedetector_raw_frames/normalised_beam_intensity[frame]

				self.raw_frame_counts = {}
				self.raw_frame_counts[frame] = self.wholedetector_raw_frames
				self.logger.log(f"Frame: {frame+1}/{self.n_frames} | Delta: {self.deltas[frame]}")

			elif (frame == None) and (sum_frames == True):
				#we want to read all frames and then sum them, ie pump-probe mode
				self.wholedetector_raw_frames  = np.zeros_like(entry["mythen_nx"]["data"][0, :, self.default_counter][()])
				...
				for frame in range(self.n_frames):
					...
					wholedetector_n_frame = entry["mythen_nx"]["data"][frame, :, self.default_counter][()]
					wholedetector_n_frame_normalised = wholedetector_n_frame/normalised_beam_intensity[frame]
					self.wholedetector_raw_frames  = self.wholedetector_raw_frames+wholedetector_n_frame_normalised
			...
			elif (frame == None) and (sum_frames == False):
				# standard data reduction mode
				self.wholedetector_raw_frames = entry["mythen_nx"]["data"][:, :, self.default_counter][()] # [n, 17920, 3] where n is the number of frames, 3 = 3 counters
				self.raw_frame_counts = {frame: self.wholedetector_raw_frames[frame,:] for frame in range(len(self.wholedetector_raw_frames))} #coverts it from list into dict
				self.logger.log(f"Deltas position(s): {self.deltas}")
				self.logger.log(f"Frames: {len(self.raw_frame_counts)}/{self.n_frames} | Specified frame: {frame}")

			self.frames_range = list(self.raw_frame_counts.keys())

			return self.raw_frame_counts, self.frames_range, self.wholedetector_raw_frames

	def load_flatfield(self, flatfield_filepath: str) -> np.ndarray:

		"""
		
		loads the flatfield file. The flatfield file is (usually) an h5 file containing a calibrated beam on the detector. This

		allows to calibrate the response of the detector to the beam ie. work out the efficiency of the detector

		the flatfield can then be used to adjust the raw counts to correct the raw data. If flatfield correction is set to False then
		...
		if self.apply_flatfield:
			...
			try:
				...
				with h5pyFile(flatfield_filepath, "r") as file:
					...
					flatfield = np.full((self.STRIPS_PER_MODULE*self.MODULES_PER_DETECTOR),0)

					for flatfield_frame in flatfield_tuple:
						flatfield +=flatfield_frame

			
			# flatfield = I11_reduction.NormalizeTo(flatfield,minval=0)
	...
	def split_flatfield(self) -> dict:
		...
		splits the raw_flatfield_counts into array of arrays, where each array corresponds to the module counts using a white beam

		"""
		if len(self.raw_flatfield_counts) == (self.STRIPS_PER_MODULE*self.MODULES_PER_DETECTOR):
		...
		elif (len(self.modules_in_flatfield) != 0) and \
		(len(self.raw_flatfield_counts) != (self.STRIPS_PER_MODULE*self.MODULES_PER_DETECTOR)) and \
		((len(self.raw_flatfield_counts)/self.STRIPS_PER_MODULE) == self.n_modules_in_data):
			flatfield_module_array = np.split(self.raw_flatfield_counts,len(self.modules_in_flatfield))
	...
	def save_nxs_outfile(self, reduced_nxs_filepath_out: str, xyedata: DataFrame, module_raw_data: DataFrame, frame_data: DataFrame, angular_corrected_data: DataFrame, debug=False) -> None:
		...
		with h5pyFile(self.filepath, "r", libver='latest', swmr=True) as file:
			...
			left_ring = file["entry"]["mythen_nx"]["data"][:, 0:17920, :][()] # [n, 17920, 3] where n is the number of frames, 3 = 3 counters
			right_ring = file["entry"]["mythen_nx"]["data"][:, 17920::, :][()] # [n, 17920, 3] where n is the number of frames, 3 = 3 counters

		with h5pyFile(reduced_nxs_filepath_out, "a", libver='latest') as out_file:

			##################################
			#just save rings exactly as they were from the hdf5 file but split into ring 1 and 2
			##################################
			...
			out_file["entry"]["xye"].create_dataset("error", data=xyedata["error"].values,dtype="f")
			out_file["entry"]["xye"].create_dataset("counts", data=xyedata["counts"].values,dtype="f")
			out_file["entry"]["xye"].create_dataset("tth", data=xyedata["tth"].values,dtype="f")
			...
			out_file["entry"]["modules"]["active_modules"] = np.array(self.active_modules)

			for column in columns_to_export:

				column_array = np.zeros((len(self.active_modules),self.STRIPS_PER_MODULE*self.n_frames))
				
				for n,module in enumerate(self.active_modules):
					module_data = tth_sorted_data[tth_sorted_data["n_mod"] == module]
					column_array[n,0:len(module_data[column].values)] = module_data[column].values
			...
			if np.amin(self.active_modules) < 14:
				ring1 = angular_corrected_data[angular_corrected_data["n_mod"].isin(np.arange(0,14,1,dtype=int))]
				for col in columns_to_export:
			...
			if np.amax(self.active_modules) > 14:
				ring2 = angular_corrected_data[angular_corrected_data["n_mod"].isin(np.arange(14,28,1,dtype=int))]
			...
			if debug:
				...
				for frame in range(self.n_frames):
					det_channel_frame = detchannel_data[detchannel_data["frame"] == frame]
					det_channel_frame_ring1 = (det_channel_frame["counts"].values)[0:17920]
					det_channel_frame_ring2 = (det_channel_frame["counts"].values)[17920::]
				...
				out_file["entry"]["frames"].create_dataset("ring1_ffcorr", data=ring1_ffcorr,dtype="f")
				out_file["entry"]["frames"].create_dataset("ring2_ffcorr", data=ring2_ffcorr,dtype="f")

				out_file["entry"]["frames"].create_dataset("ring1_ffcorr_flipped", data=ring1_ffcorr_flipped,dtype="f")
				out_file["entry"]["frames"].create_dataset("ring2_ffcorr_flipped", data=ring2_ffcorr_flipped,dtype="f")
	...
	def align_modules_dict(self, deltas: np.ndarray) -> dict:
		...
		Creates a self.all_module_data which ia a dicitionary containing delta position and module datae broken up into an accessable way

		self.all_module_data[n_frame][n_mod]

		n_frame may relate to a delta position or potentially the same delta if running a time resolved experiment

		If flatfield correction is True in the config file it will apply the flatfield correction

		"""

		self.all_module_data = {} #is a dictionary containing a load of dataframes. Each dict is self.all_module_data[n_frame][n_mod] = ModuleDataAtDelta

		det_channels =  np.arange(self.STRIPS_PER_MODULE*len(self.active_modules),dtype=np.int64)
		...
		end_bad_chans = np.arange(self.STRIPS_PER_MODULE-self.n_bad_edge_channels,self.STRIPS_PER_MODULE,1)

		for n_frame,delta_pos in zip(self.frames_range, deltas):

			self.all_module_data[n_frame] = {}
			counts_in_each_frame_and_module = np.array_split(self.raw_frame_counts[n_frame],len(self.active_modules))
			...
			for n,(n_mod) in enumerate(self.active_modules):
				...
				ModuleDataAtDelta = DataFrame()
				ModuleDataAtDelta["raw_tth"] = self.module_raw_tth[n_mod]
				ModuleDataAtDelta["tth"] = ModuleDataAtDelta["raw_tth"].values+delta_pos
				ModuleDataAtDelta["mod_channel"] = self.module_raw_tth["mod_channel"] #always 0-1280 for each module
				ModuleDataAtDelta["det_channel"] = module_det_channels[n]
				ModuleDataAtDelta["frame"] = n_frame
				ModuleDataAtDelta["delta_pos"] = delta_pos
				ModuleDataAtDelta["n_mod"] = n_mod
				ModuleDataAtDelta["no_ff_corr"] = counts_in_each_frame_and_module[n]

				#if flatfield is not enabled, the entire array is set to 1, so it makes no difference

				# self.logger.log(n_mod, self.flatfield_modules[n_mod])
				ModuleDataAtDelta["counts"] = self.apply_flatfield_correction(ModuleDataAtDelta["no_ff_corr"].values,self.flatfield_modules[n_mod])

				# ModuleDataAtDelta["counts"] = self.apply_flatfield_correction(ModuleDataAtDelta["no_ff_corr"].values,self.flatfield_modules[n_mod])
				ModuleDataAtDelta["error"] = np.sqrt(ModuleDataAtDelta["counts"]) #poisson errors

				#if isinstance(self.bad_channels,dict)
				# ModuleDataAtDelta["badchannel"] = ( (ModuleDataAtDelta['counts'] == 0) \
				# | (ModuleDataAtDelta["mod_channel"].isin(self.bad_channels[n_mod])) \
				# | (ModuleDataAtDelta["mod_channel"].isin(start_bad_chans)) \
				# | (ModuleDataAtDelta["mod_channel"].isin(end_bad_chans)) ) old way

				#if isinstance(self.bad_channels,np.ndarray):


				ModuleDataAtDelta["badchannel"] = ( (ModuleDataAtDelta["det_channel"].isin(self.bad_channels)) \
				| (ModuleDataAtDelta["mod_channel"].isin(start_bad_chans)) \
				| (ModuleDataAtDelta["mod_channel"].isin(end_bad_chans)) ) #new way

				self.all_module_data[n_frame][n_mod] = ModuleDataAtDelta


		

			# 	plt.plot(ModuleDataAtDelta["tth"],ModuleDataAtDelta["counts"])
			# 	plt.plot(ModuleDataAtDelta["tth"],ModuleDataAtDelta["no_ff_corr"])
			# plt.show()


		return self.all_module_data

	def remove_bad_channels_modules_frames(self) -> DataFrame:

		if self.bad_channel_masking:

			angular_corrected_data = self.angular_corrected_data_unmasked[(~self.angular_corrected_data_unmasked["n_mod"].isin(self.bad_modules)) \
			& (self.angular_corrected_data_unmasked["badchannel"] == False)\
			& (~self.angular_corrected_data_unmasked["frame"].isin(self.bad_frames)) ] #keep non-bad modules #remove bad channels
		
		else:

			angular_corrected_data = self.angular_corrected_data_unmasked[(~self.angular_corrected_data_unmasked["n_mod"].isin(self.bad_modules))\
			& (~self.angular_corrected_data_unmasked["frame"].isin(self.bad_frames))] #keep non-bad modules only

		return angular_corrected_data
	
	
	
	def concatenate_frames_and_modules(self) -> DataFrame:

		"""
	
		it contatenates the data in self.all_module_data and then appends it into a dataframe containing lots of info. Then remove the bad channels

		I suspect when fast_shutter_mode is available we would probably want to iterate frames first, modules second and build a self.frame_outputdata dict instead of modules

		This way we could iterate the frames and save an xye for each frame. Which means we wouldn't concatenate everything into a dataframe, we would concatenate all 
		...
		angular_corrected_data_unmasked = DataFrame()

		for n_mod,n_frame in product(self.active_modules,self.frames_range):

			angular_corrected_data_unmasked = concat([angular_corrected_data_unmasked,self.all_module_data[n_frame][n_mod]],axis=0)

		angular_corrected_data_unmasked = angular_corrected_data_unmasked.sort_values(by="tth",ascending=True)
	...
	def check_active_modules(self) -> bool:
		...
		checks to see if the modules specified in the config are correct for the data shape. If not it will try to infer the correct shape.

		"""

		self.logger.log(f"\nModules in data: {self.n_modules_in_data}")

		if (len(self.active_modules) != self.n_modules_in_data) and (os.path.exists(self.mythen3_config_dir)):
			self.logger.log(f'\nNumber of active modules in {self.config_filepath} does not reflect data!!!!')
			self.logger.log("Modules will be determined from mythen3 config")
			self.active_modules = I11_reduction.read_config(self.mythen3_config_dir)


		if (len(self.active_modules) != self.n_modules_in_data) and (self.n_modules_in_data == 14):
		...
		elif (len(self.active_modules) != self.n_modules_in_data) and (self.n_modules_in_data == 28):
			self.logger.log("Modules will be determined by using a range()")
			self.active_modules = np.arange(self.n_modules_in_data,dtype=int)
		...
		if len(self.active_modules) != self.n_modules_in_data:
			self.logger.log(f"The modules must be specified correctly in active_modules in: {self.config_filepath}")
			quit()
	...
	def calculate_wavelength(beam_energy: float) -> float:
		...
		To allow convertion of tth to Q space, using the energy of the beam. beam energy is converted to wavlength because it's better

		"""

		beam_energy_ev = beam_energy*1000
		ev_to_J, h_planck, c_speed_of_light = 1.602176634e-19, 6.62607015e-34, 299792458.0 #electron volt-joule relationship (in J), plancks constant and speeed of light (m/s)
	...
	@staticmethod
	def convert_tth_to_Q(tth: np.ndarray, wavelength: float) -> np.ndarray:
	...
	def bin_and_propagate_errors(self, x: np.ndarray, y: np.ndarray ,e: np.ndarray, error_calc: str ='best') ->  np.ndarray:
		...
		The bin centres and edges are calculated and used to bin the data. Binning of the data is done used searchsorted == np.digitize.

		Because we want to propagate the errors we will iterate though all the values of x, y and e that need to be binned together and propagate the errors

		Errors can be calculated using internal error = error propagation, external error std_dev of error or we can take the greatest of the two values. Which is probabaly the best idea.

		If you have a high spread of data (high noise), ie peaks with weak intensity surely the error can't be less than the spread. But equally if you have very large peaks with low spread the
		...
		if x[-1] == bin_edges[-1]: #if the last value is exactly equal to the final bin edge it will be lost. 
			x[-1] = x[-1]-(self.rebin_step/10000) #I think it would be better to move it inside bin edge, and include, rather than remove all together or create a bin with a single value

		sums, bin_edges = np.histogram(x, bins=bin_edges, weights=y)
		counts = np.histogram(x, bins=bin_edges)[0]
		mean_counts = sums / counts #this will throw a warning if there are missing counts in a bin as a result of missing module


		e_sums = np.histogram(x, bins=bin_edges, weights=e**2)[0] #https://faraday.physics.utoronto.ca/PVB/Harrison/ErrorAnalysis/Propagation.html
	...
	def bin_data(self, angular_corrected_data: DataFrame, error_calc: str) -> DataFrame:

		"""
	
		creates the bins, fins the bins centres (which gives the 2th angle) Then sticks this all in a dataframe called xyedata with tth, counts, error and if wavelength is provided Q
	...
	def save_xye(self, xye_filepath_out: str, xyedata: DataFrame, x: str ) -> np.ndarray:

		"""

		given a filepath out, and a dataframe containing some x axis (Q or tth) and an x axis label 
	...
	def split_data(self, unmasked: bool = False):
		...
		splits data into frames and modules, depending on how you want to look at the data, creates self.frame_data and self.module_raw_data
		...
		frame_data = {frame: data_to_split[data_to_split['frame'] == frame] for frame in data_to_split.frame.unique()}
		module_raw_data = {n_mod: data_to_split[data_to_split['n_mod'] == n_mod] for n_mod in data_to_split.n_mod.unique()}
	...
	def load_toml_config(self) -> bool:
		...
		Loads the config from a toml file for the data reduction. Returns true when success.
		...
		if self.angcal_filepath == None:
			self.angcal_filepath = self.config["angcal_filepath"]

		if self.data_reduction_mode == None:
		...
		self.logger.log("Number of bad channels at module edge:",self.n_bad_edge_channels)
		self.logger.log("Rebin step:",self.rebin_step)

		data_reduction_mode_dict = {0: "standard", 1: "time-resolved", 2: "pump-probe", 3: "flatfield"}

		self.logger.log("Data reduction mode:",data_reduction_mode_dict[self.data_reduction_mode])

		return True

	# def bin_frames_and_save(self):

	# 	for n_frame in self.frames_range:

	# 		frame_angular_corrected_data = self.frame_data[n_frame]
	# 		xye_frame_out = self.bin_data(frame_angular_corrected_data)
	# 		self.save_xye(self.xye_filepath_out.replace(".xye",f"_frame_{n_frame}.xye"), xye_frame_out, "tth")
	...
	def set_save_filepaths(self) -> bool:
		...
		if not self.xye_filepath_out:
			self.xye_filepath_out = os.path.join(self.file_dir,f"{self.file_name}_summed_mythen3{self.filename_suffix}.xye")
		...
		if not self.reduced_nxs_filepath_out:
			self.reduced_nxs_filepath_out = os.path.join(self.file_dir,"processed",f"{self.file_name}_reduced_mythen3{self.filename_suffix}.nxs")

		if not self.xye_filepath_out_Q:
			self.xye_filepath_out_Q= os.path.join(self.file_dir,f"{self.file_name}_summed_mythen3_Q{self.filename_suffix}.xye")
	...
	def _refine_rebin(self):
		...
		# rebinned_tth, tth_bin_edges, rebinned_counts, errors = self.bin_and_propagate_errors(self.angular_corrected_data["tth"].values,
		# 																		self.angular_corrected_data["counts"].values, 
		# 																		self.angular_corrected_data["error"].values,'internal')
	...
	def plot_by(self, parameters=[], x="tth", at_a_time=False):
		...
		for parameter in parameters:
			...
			for val in np.unique(self.angular_corrected_data[parameter]):
				...
				parameter_data =  self.angular_corrected_data[self.angular_corrected_data[parameter] == val]
				p = plt.scatter(parameter_data[x], parameter_data["counts"],label=str(val),s=2)
				col = p.get_facecolors()[-1].tolist()
				plt.text(np.median(parameter_data[x]), -np.amax(self.angular_corrected_data["counts"])/30, str(val), color=col)
	...
	def plot_module_offsets(self, mask=()):
		...
		for x,y in zip(ring_1_angles.keys(), ring1_steps):
			print(x,y)

		for x,y in zip(ring_2_angles.keys(), ring2_steps):
			print(x,y)
	...
	def plot_diffraction(self, out_only=True, filepath=None):
		...
		plt.errorbar(self.xyedata["tth"],self.xyedata["counts"],self.xyedata["error"],label=self.error_calc)
		plt.legend()
		plt.xlabel("tth")
		plt.ylabel("Intensity (arb. units)")
	...
	def plot_raw_vs_xye(self):
		...
		axes[0].step(self.angular_corrected_data["tth"],self.angular_corrected_data["counts"],label="ff corrected counts", color='k')
		axes[0].plot(self.xyedata["tth"],self.xyedata["counts"],label="xye")
		axes[0].legend()

		axes[1].step(self.angular_corrected_data["tth"],self.angular_corrected_data["no_ff_corr"],label="ff uncorrected counts", color='k')
	...
	def plot_rings(self):
		...
		ringmod0_13 = self.angular_corrected_data[self.angular_corrected_data["n_mod"].isin(np.arange(0,14,1,dtype=int))]
		ringmod14_27 = self.angular_corrected_data[self.angular_corrected_data["n_mod"].isin(np.arange(14,28,1,dtype=int))]
		plt.errorbar(ringmod0_13["tth"],ringmod0_13["counts"],ringmod0_13["error"],label='ring1')
		plt.errorbar(ringmod14_27["tth"],ringmod14_27["counts"],ringmod14_27["error"],label='ring2')
		plt.show()
		plt.close()
	...
	def return_outliers(self, factor: int | float = 3, low_bound: float | None = None, plot: bool = False) -> None:

		"""
		This is used to return bad pixels.
		This should only be run on a scan conducted on water at high angles (or something else that scatters very flat) - otherwise the results are invalid"""
		...
		for n_mod in np.unique(self.angular_corrected_data["n_mod"]):
			...
			if plot:
				...
				plt.scatter(bad_channels, [median_count]*len(bad_channels),color="red")
				plt.legend()
				plt.show()
	...
	# def debug_reduction(self):

	# 	self.logger.log('Saving debug nxs...')

	# 	unmasked_frame_data, unmasked_module_raw_data = self.split_data(unmasked=True) #if this is done after remove bad channels and modules
	# 	unmasked_xyedata = self.bin_data(self.angular_corrected_data_unmasked,error_calc=self.error_calc)
	# 	debug_reduced_nxs_filepath_out = self.reduced_nxs_filepath_out.replace('.nxs','_debug.nxs')
	# 	self.save_nxs_outfile(debug_reduced_nxs_filepath_out, unmasked_xyedata, unmasked_module_raw_data, unmasked_frame_data, self.angular_corrected_data_unmasked, debug=True)

	# def ffcorr_calc(self, wholedetector_raw_frames):
		
	# 	flipped_data = np.zeros_like(wholedetector_raw_frames)

	# 	for exp,frame_data in enumerate(wholedetector_raw_frames):

	# 		frame_mod_data = np.array_split(frame_data, len(self.active_modules))
	...
	# 			if (n_mod <= 13):
	# 				pos = 13-n_mod
	# 				ffcorr = frame_mod_data[n]

	# 			else:
	# 				pos = n_mod
	# 				ffcorr = np.flip(frame_mod_data[n],axis=0)

	# 			start = (pos)*self.STRIPS_PER_MODULE
	# 			stop = ((pos+1)*self.STRIPS_PER_MODULE)

	# 			self.logger.log(len(ffcorr))
	# 			self.logger.log(exp)
	# 			self.logger.log(start,stop)

	# 			self.logger.log(np.shape(flipped_data))
	...
	def data_reduction_mode_standard(self):
		...
		self.raw_frame_counts, self.frames_range, self.wholedetector_raw_frames = self.read_nxs_data(self.filepath)
		# self.ffcorr = self.ffcorr_calc(self.wholedetector_raw_frames)


		self.all_module_data = self.align_modules_dict(self.deltas) #its a dict of dataframes
		...
		self.frame_data, self.module_raw_data = self.split_data(unmasked=self.out_raw_data) #if this is done after remove bad channels and modules
		self.xyedata = self.bin_data(self.angular_corrected_data,error_calc=self.error_calc)
		...
		if self.save_nxs_out:
			# try:
			self.save_nxs_outfile(self.reduced_nxs_filepath_out, self.xyedata, self.module_raw_data, self.frame_data, self.angular_corrected_data_unmasked,debug=self.debug_mode)
			# except:
	...
	def data_reduction_mode_time_resolved(self):
		...
		#This iterates through every frame and load them seperately because for large datasets it will eat up memory

		for n_frame in range(self.n_frames):

			self.logger.log(f"Analysing frame: {n_frame}")

			self.raw_frame_counts, self.frames_range, self.wholedetector_raw_frames = self.read_nxs_data(self.filepath,frame=n_frame)
			self.all_module_data = self.align_modules_dict([self.deltas[n_frame]]) #align all modules for the specfic frame of data
			self.angular_corrected_data_unmasked = self.concatenate_frames_and_modules()
			self.angular_corrected_data =  self.remove_bad_channels_modules_frames()
			self.frame_data, self.module_raw_data = self.split_data(unmasked=self.out_raw_data) #if this is done after remove bad channels and modules
			self.xyedata = self.bin_data(self.angular_corrected_data,error_calc=self.error_calc)

			self.save_xye(self.xye_filepath_out.replace(".xye",f"_frame_{n_frame+1}{self.filename_suffix}.xye"), self.xyedata, "tth")
			if (self.save_in_Q_space) and (self.beam_energy):
				self.save_xye(self.xye_filepath_out_Q.replace(".xye",f"_frame_{n_frame+1}{self.filename_suffix}.xye"), self.xyedata, "Q")

	def data_reduction_mode_pump_probe(self):

		#pump probe

		self.raw_frame_counts, self.frames_range, self.wholedetector_raw_frames = self.read_nxs_data(self.filepath,sum_frames=True)
		self.all_module_data = self.align_modules_dict(self.deltas) #its a dict of dataframes
		self.angular_corrected_data_unmasked = self.concatenate_frames_and_modules()
		self.angular_corrected_data =  self.remove_bad_channels_modules_frames()
		self.frame_data, self.module_raw_data = self.split_data(unmasked=self.out_raw_data) #if this is done after remove bad channels and modules
		...
		if self.save_nxs_out:
			self.save_nxs_outfile(self.reduced_nxs_filepath_out, self.xyedata, self.module_raw_data, self.frame_data, self.angular_corrected_data,debug=self.debug_mode)
	...
	def data_reduction_mode_flatfield(self, cleanup=True, peak_centre=80.67, tol=3.05):
		...
		#flatfield
		'''
		This assumes that the angular calibration is correct, if it isn't then the peak centre will be wrong and the flatfield will be wrong.
		...
		# throw away bad channels, and find the peak maxima. If this is a flatfield, that peak will always be at that tth
		#

		self.raw_frame_counts, self.frames_range, self.wholedetector_raw_frames = self.read_nxs_data(self.filepath,frame=int(self.n_frames/2))
		self.all_module_data = self.align_modules_dict([self.deltas[int(self.n_frames/2)]]) #its a dict of dataframes
		self.angular_corrected_data_unmasked = self.concatenate_frames_and_modules()
		self.angular_corrected_data =  self.remove_bad_channels_modules_frames()
		...
		for n_frame in range(self.n_frames):
			...
			self.logger.log(f"Analysing frame: {n_frame}, Beam: {normalised_beam_intensity[n_frame]}")
			self.raw_frame_counts, self.frames_range, self.wholedetector_raw_frames = self.read_nxs_data(self.filepath,frame=n_frame)

			beam_normalised_flatfield = self.wholedetector_raw_frames/normalised_beam_intensity[n_frame]

			if cleanup:
				tth = self.deltas[n_frame] + self.whole_data_raw_tth
				tth_throw_index = np.where( ~((tth < (peak_centre+tol)) & (tth > (peak_centre-tol))))[0]
				beam_normalised_flatfield[tth_throw_index] = 0

				# beam_normalised_flatfield = beam_normalised_flatfield - np.amin(beam_normalised_flatfield)
		...
		# new_flatfield[count_throw_index] = (new_flatfield[count_throw_index-1]+new_flatfield[count_throw_index+1])/2
		...
		flatfield_dir_flatfield_save_path = os.path.join(save_dir,f"{self.file_name}_flatfield_{datetimestr}{self.filename_suffix}.h5")


		with h5pyFile(flatfield_dir_flatfield_save_path, "w") as out_file:
			dset = out_file.create_dataset("flatfield", data=new_flatfield)

		self.logger.log(f"New flatfield has been saved to {flatfield_dir_flatfield_save_path}")
	...
	def data_reduction_mode_0_fast(self):
		...
		for n_frame, delta in range(self.n_frames), self.deltas:
			...
			self.raw_frame_counts, self.frames_range, self.wholedetector_raw_frames = self.read_nxs_data(self.filepath, frame=n_frame)
			self.all_module_data = self.align_modules_dict([delta]) #its a dict of dataframes
	...
	def communicate_with_control(self, send_to_ispyb: bool = False):
		...
		"""
		Attempts to connect to i11-control and send a message indicating that a file has been processed. This will cause gda to plot the latest file
		...
		try:
			...
			daq.connect()
			# self.logger.log(f"sending {self.reduced_nxs_filepath_out}, stomp is old? {daq.old_stomp}")
	...
	def __init__(self, 
		...
		if self.bad_frames == None:
			self.bad_frames = [] #frames that should be removed, because they are bad for whatever reason


		if self.out_directory != None:
			self.file_dir = self.out_directory # this will replace the directory with the one specified by the user
		else:
			self.file_dir = os.path.dirname(self.filepath) # this will be something like /dls/i11/data/2025/cm40643-1


		self.logger = AnalysisLogger(os.path.join(self.file_dir,"processed", "mythen3_reduction.log"))
		self.logger.log("######################################\n")
		self.logger.log(f"Data reduction being performed on: {self.filepath}")



		self.file_name = os.path.splitext(os.path.basename(self.filepath))[0] # this will be something like 1290222
		...
		if not self.config_filepath:
			self.config_filepath = '/dls_sw/i11/software/mythen3/diamond/mythen3_reduction_config.toml'
		...
		if self.beam_energy:
			self.wavelength = I11_reduction.calculate_wavelength(self.beam_energy)
			self.logger.log(f"Beam Energy: {self.beam_energy} (keV) | Wavelength = {self.wavelength:.3f} (Angstrom)\n")


		# self.bad_channels = self.generate_badchannel_dict()
		self.bad_channels = I11_reduction.load_int_array_from_file(self.bad_channels_filepath)	

		if self.angcal_filepath:
			self.logger.log(f"Using the following angular calibrations file: {angcal_filepath}")

			# if self.angcal_filepath.endswith(".json"):
			# 	self.module_angular_cal, self.beamline_offset = self.read_angular_cal_json(self.angcal_filepath)
			# else:
			self.module_angular_cal, self.beamline_offset =  I11_reduction.read_singular_angcal_files(self.angcal_filepath)
		else:
			...
			self.module_angular_cal, self.beamline_offset =  I11_reduction.read_angular_calibration_and_create_cal_dict(self.config, self.active_modules)

			
		#####################################################################################################################################
		#everything before here is just setting up calibrations, and hasn't read the actual dataset

		
		if self.filepath.lower().endswith('.nxs'):
			self.n_frames, self.deltas, self.n_modules_in_data = self.read_nxs_metadata(self.filepath)
		else:
		...
		if self.execute_reduction:
			...
			if (self.data_reduction_mode == 0) and (self.n_frames > 50) and ( (np.amax(self.deltas) - np.amin(self.deltas)) > 60):
				self.logger.log("This looks like a flatfield scan...treating it as such")
			...
			if (self.data_reduction_mode == 0):
				...
				self.logger.log("Data reduction mode must be one of the specified values")
			...
			zeros = (self.angular_corrected_data[self.angular_corrected_data["counts"] == 0]).sort_values(by="det_channel",ascending=True)
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(
		description="Post-processor for mythen3 data; converts an uncalibrated .nxs files "
		"written by the detector into a calibrated and corrected .xye ASCII file.",add_help=True)
	...
	parser.add_argument(
		"-c", "--config", help="Path to .toml config file, containing the bad channels, angular cal files, binning size, etc", required=False
	)

	parser.add_argument(
		"-o", "--out-xye-file", help="Path to write output .xye file (2th, counts, error)", required=False
	)


	parser.add_argument(
		"-q", "--out-q-space-file", help="Path to write output Q .xye file (Q, counts, error)", required=False
	)

	parser.add_argument(
		"-nxs", "--out-nxs-file", help="Path to write output processed .nxs file", required=False
	)

	parser.add_argument(
		"-ang", "--ang-cal-file", help="Path to angular calibration file", required=False
	)

	parser.add_argument(
		"-drm", "--data-reduction-mode", help="How the data should be reduced. 0, 1 or 2 \
		0 = standard (all data will be reduced into 1 file, possibly multiple angles) \
		1 = time-resolved mode (many frames per .nxs saved into seperate .xye files for each frame)\
		2 = pump-probe (all frames will be read and summed together - data has been taken at static angle, optimised for lots of frames)", required=False
	)

	parser.add_argument('-bf', '--bad_frames', help="A list of 'bad frames' that will be removed from the final dataset. Specified comma seperated eg. -bf 0,1,2,3 ", required=False, 
		type=lambda s: [int(item) for item in s.split(',')]
		)


	parser.add_argument(
		"-l", "--live", help="Is this a live experiment, and therefore shuold we send messenges to gda?", required=False
	)
	...
	import time
	start = time.time()
	Analysis = I11_reduction(filepath=args.data,config_filepath=args.config, xye_filepath_out_Q=args.out_q_space_file,reduced_nxs_filepath_out=args.out_nxs_file, angcal_filepath=args.ang_cal_file,live=args.live)
	end = time.time()
